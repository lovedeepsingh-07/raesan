...
async fn cancel_populate(
    state: tauri::State<'_, RwLock<state::AppState>>,
) -> Result<(), error::Error> {
...
async fn populate_database(
    ...
    let _guard = state::AppStateGuard(state.inner());
...
pub fn run() {
    tauri::Builder::default()
        .setup(|app| {
            app.manage(RwLock::new(state::AppState::new()));
            Ok(())
        })
        .plugin(tauri_plugin_opener::init())
        .invoke_handler(tauri::generate_handler![populate_database, cancel_populate])
